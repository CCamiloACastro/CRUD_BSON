...
# crud.create_many(documents) # Funciona con json pero no con bson
...
# crud.read_many({}) # Sin criterio nos devuelve todos los objetos
# crud.read_many({"edad": {"$lt": 30}})
# crud.read_many({"edad": {"$eq": 32}})
# crud.read_many({"edad": {"$gt": 25}})
# crud.read_many({ "edad": {"$gte": 19, "$lte": 40}})
# crud2.read_many({"edad": {"$eq": 33}})


# Delete
# crud.delete_one_doc({"nombre": "Juan"})
# crud.delete_many_doc({"nombre": "Pepito"})
